<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
    <a class="navbar-brand" href="#">
        <img src="img/poke.png" alt="Logo" style="height: 60px; margin-right: 40px;">
        Sistema de ventas
    </a>
</nav>


<div class="siderbar">
    <a href="admin.php"><i class="fa fa-tachometer-alt mr2"></i> Inicio</a>
    <a href="cliente.php"><i class="fa fa-users mr2"></i> Cliente</a>
    <a href="producto.php"><i class="fa fa-box-archive mr2"></i> Producto</a>
    <a href="opciones-prod.php"><i class="fa fa-box-archive mr2"></i> Opciones Producto</a>
    <a href="venta.php"><i class="fa-solid fa-store mr2"></i> Ventas</a>
    <a href="#"><i class="fa-solid fa-cart-shopping mr2"></i> Compras</a>
    <a href="usuario.php"><i class="fa-solid fa-user-tie mr2"></i> Usuarios</a>
    <a href="#"><i class="fa fa-file mr2"></i> Reportes</a>
    <a href="#"><i class="fa fa-cogs mr2"></i> Configuración</a>
    <a href="index.php"><i class="fa fa-sign-out-alt mr2"></i> Cerrar Sesión</a>
</div>